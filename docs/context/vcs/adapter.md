...
- **Reply-on-comment trigger (OPEN — not wired/tested):** build validation fires on PR create/update but **not** on comments, so the reply flow isn't triggered in CI. The FSM already decides review-vs-reply in one entrypoint, so this is trigger wiring, not agent code. Plan: Service Hook on `ms.vss-code.git-pullrequest-comment-event` → Azure Pipelines **Incoming Webhook** (`resources.webhooks`) → a reply pipeline reading `resource.pullRequest.pullRequestId` from the payload (`System.PullRequest.*` is unset on webhook runs). Open: agent-authored comments self-fire a cheap no-op run (FSM dedups by footer/timestamp — no loop) → accept or filter?; verify the `resources.webhooks` payload-path syntax against a real payload; hybrid (build-validation reviews + webhook replies) vs one all-webhook pipeline. Comment-command triggers (`/azp run`) are GitHub-only, not usable for Azure Repos.
