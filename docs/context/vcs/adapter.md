...
`src/vcs/azure.ts` — experimental. Purely additive: no existing adapter, the interface, or `review/*` was touched. **Validated end-to-end against a live Azure DevOps Services (cloud) org** — `getPullRequestInfo`, diff reconstruction, `getChangedFiles`, `getFileContent` context, `getRepoFileContent`, `postComment`, and footer dedup (`getPreviousReviewComments`) all confirmed on a real PR (dry-run → post → dedup-skip). Still WIP: on-prem **Server**, a real pipeline `System.AccessToken` run, and the `getCommitDiff`-delta path are unverified. The reply flow (below) is now validated live.
...
- **Reply-on-comment trigger (validated live, WIP):** build validation fires on PR create/update but **not** on comments, so the reply flow needs a second trigger. The FSM already decides review-vs-reply in one entrypoint, so this is trigger wiring, not agent code — captured in [azure-reply-flow-runbook.md](../../reference/azure-reply-flow-runbook.md) + [`azure/azure-reply-pipeline.yml`](../../../azure/azure-reply-pipeline.yml): Service Hook on `ms.vss-code.git-pullrequest-comment-event` → Azure Pipelines **Incoming Webhook** (`resources.webhooks`) → a reply pipeline reading `resource.pullRequest.pullRequestId` from the payload (`System.PullRequest.*` is unset on webhook runs). **Validated end-to-end on an Azure DevOps Services test org**: comment → Service Hook → Incoming Webhook → reply pipeline → agent posts a reply, confirmed. The Incoming Webhook secret can be left **blank** (no HMAC needed) and still fires — resolves what was the biggest open question. Agent-authored comments self-fire a cheap no-op run (FSM dedups by footer/timestamp — no loop); hybrid (build-validation reviews + webhook replies) kept, not unified. Comment-command triggers (`/azp run`) are GitHub-only, not usable for Azure Repos. **Review-vs-reply priority:** the FSM handles new commits before comments — a comment on a PR with unreviewed new commits triggers a delta review, not a reply; a reply posts only when the current commit is already reviewed and a human question is unanswered in a review thread. **Throttling:** none native (every comment = one pipeline run); skip agent-authored comments via a pipeline condition if the no-op runs get noisy.
